...
| 🟡 CP-051 | Frontend quality and automated tests | Vitest + React Testing Library set up from scratch (`vite.config.js`'s `test` block, `frontend/tests/setup.js`) — deliberately placed under a top-level `frontend/tests/` rather than co-located next to each component (the more common JS-ecosystem convention), matching this project's own existing pattern of a dedicated `tests/` folder per service rather than mixing test files into application code. Now wired into `.github/workflows/ci.yml`'s frontend job (`npm test` runs before `npm run build`). First real component tests: `ProductImage.test.jsx` (3 tests — a direct regression test for the broken-image-URL bug found live this session: real image renders, no `src` shows the placeholder, and a failed image load falls back to the placeholder rather than a permanent broken icon) and `ProductCard.test.jsx` (6 tests — stock-based Add button enable/disable, the 404-vs-500 distinction in the out-of-stock logic, and that clicking Add doesn't also open the product via event propagation). Hit and fixed two real tooling issues, not test-code issues, on the very first run: Vitest's default "forks" process pool timed out starting workers in this environment (switched to `pool: "threads"`), and React Testing Library needs an explicit `afterEach(cleanup)` under Vitest — it isn't automatic the way it is under Jest, and every test silently saw leftover DOM from the previous one until this was added. All 9 tests pass, production build unaffected (test files are naturally excluded, nothing extra needed). | Two components with real, meaningful behaviour, not the whole frontend — no tests yet for `CartPage` (form validation, payment-method switching), `CustomersOrdersPage` (delivery-status dropdown gating), `AdminPanel`, or any routing/navigation logic. No accessibility-specific checks (e.g. axe). No responsive-layout tests. |
| 🟡 CP-041 | Final report | **Was wrongly marked ⬜ — a real, substantial draft already exists** at `docs/SmartRetailX_Technical_Report_Draft.docx` (~4,300 words, within the brief's 4,000–5,000 target), found during a 2026-08-18 audit against the actual assignment spec. Follows the brief's exact requested section structure, Harvard-referenced, and is explicit and honest throughout about what's implemented vs deferred (e.g. "CHECK BEFORE SUBMISSION" notes on ECS Fargate, labelled screenshot placeholders). **On the NEVER CUT list.** | Written before tonight's work, so three claims in it are now stale and need correcting: it says the frontend "has not yet been uploaded" (it has, repeatedly, since), and that security testing and UI testing are "still needed" (both now real — CP-035, CP-051). Also still needed regardless: every `[INSERT SCREENSHOT n]` placeholder replaced with real evidence, the ECS Fargate section resolved one way or the other, word count/references finalised, export to PDF. |
| 🟡 CP-052 | Complete API documentation | **Also wrongly marked ⬜ — two of four already exist.** `docs/api/inventory-service.openapi.json` and `docs/api/payment-service.openapi.json` are real exported OpenAPI definitions, found in the same audit. | `product-service` and `order-service` still need their OpenAPI definitions exported. None of the four yet document auth requirements, roles, pagination, error shapes, or (once built) WebSocket message contracts. |
...
12. **A 2026-08-18 audit against the actual assignment specification (`docs/COMP60010-ECDWA2_Assignment_v20260510.docx`, extracted and read directly, not just `PROJECT_BRIEF.md`'s summary) found this checklist itself was wrong in two places, and named the real remaining gaps precisely.**
    `CP-041` (final report) and `CP-052` (API documentation) were both marked ⬜ with no
    knowledge that real, substantial work already existed for both — a ~4,300-word report draft
    and two of four OpenAPI exports — simply because nobody had looked in
    `docs/SmartRetailX_Technical_Report_Draft.docx` or `docs/api/` before marking them unstarted.
    Both corrected to 🟡. Cross-checking every task in the actual brief against what exists also
    confirmed, precisely rather than by general impression, that the single largest unaddressed
    item is Task 1's explicit requirement for **"Kubernetes (Amazon EKS) or ECS Fargate"** —
    named separately from and in addition to Lambda, and completely absent from the current
    architecture (CP-032). Task 4's "WebSockets or server push mechanisms" (CP-020) is the second
    most structurally significant gap, for the same reason: named explicitly, not implemented at
    all. Worth recording precisely because a checklist that has not been checked against its own
    source document can drift silently — this is the same lesson as discrepancies 5 and 7, applied
    to the tracking document itself rather than to code or infrastructure.
