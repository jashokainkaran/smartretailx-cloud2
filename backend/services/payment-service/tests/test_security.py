...
def lambda_event(method, path, claims=None):
    event = {
        "version": "2.0",
...
def test_no_claims_at_all_is_rejected(monkeypatch):
    ...
    response = handler(lambda_event("GET", "/api/v1/payments/does-not-exist"), {})
...
def test_customer_on_admin_only_route_is_rejected(monkeypatch):
    ...
    response = handler(lambda_event("GET", "/api/v1/payments/does-not-exist", claims=claims), {})
...
def test_admin_single_group_bracket_string_is_accepted_end_to_end(monkeypatch):
    ...
    404, not 401/403, is the proof: auth passed and reached the real
    "payment not found" business logic."""
    monkeypatch.setattr(config, "AUTH_TEST_MODE", False)
